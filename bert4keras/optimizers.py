...
# 新增自定义优化器
@export_to_custom_objects
def extend_with_noam_decay(BaseOptimizer):
    """ noam 学习率衰减策略
    # 参考 attention is all you need
      <https://arxiv.org/pdf/1706.03762.pdf>
    """
    class NewOptimizer(BaseOptimizer):
        @insert_arguments(warmup_steps=10000)
        def __init__(self, *args, **kwargs):
            super(NewOptimizer, self).__init__(*args, **kwargs)
        ...
        def get_updates(self, loss, params):
            ...
            iterations = K.cast(self.iterations + 1, K.floatx())
            warmup_steps = K.cast(self.warmup_steps, K.floatx())
            a = iterations * warmup_steps ** (-1.5)
            b = iterations ** (-0.5)
            lr_multiplier = K.minimum(a, b)
            ...
            self.llr = lr_multiplier * self.lr
